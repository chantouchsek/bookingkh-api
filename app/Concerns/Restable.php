...
trait Restable
{
     ...
     */
    public function respondWithMessage($data, $message = 'Item has been created.', $headers = [])
    {
        $data = array_merge(
            $data, [
                'success' => [
     ...
     * @param string $status The given status
     * @param array $headers The headers that should be send with the JSON-response
     * @param array $errors The errors that should be response from validation
     *
     * @return JsonResponse The JSON-response with the message
     */
    public function respondJson($message, $status = 'success', $errors = [], $headers = [])
    {
        $data = [
            'status' => $status,
            'message' => $message,
